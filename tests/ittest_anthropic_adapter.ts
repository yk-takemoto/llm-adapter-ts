...
// APIキーが設定されているか確認
const requireEnvVars = ["ANTHROPIC_API_KEY", "ANTHROPIC_API_MODEL_CHAT"];
...
describe("Anthropic API 統合テスト", function () {
  ...
  describe("chatCompletions インテグレーションテスト", () => {
    it("通常のテキスト会話が正しく処理されること", async function () {
      ...
      const anthropicAdapter = anthropicAdapterBuilder.build();
      const result = await anthropicAdapter.chatCompletions({
        args: {
